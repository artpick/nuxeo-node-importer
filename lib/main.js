  ...
  function createDocumentFromFile(file, remotePath) {
    ...
    var documentToUpload = {
      ...
      properties: {
        ...
        "visual:name": path.basename(file)
      }
    }

    return nuxeo.repository()
    .create(remotePath, documentToUpload)
      .then((documentCreated) => {
        ...
        return nuxeo.operation('Blob.AttachOnDocument')
            .input(blob) 
            .param('document', documentCreated.path)
            .execute();
      })
      .then((doc) => {
        if (config.verbose) {
        ...
        if (config.verbose) {
          console.log('Taks in error for: ' + file);
        }
        foldersNotWellProcessed.push({ path: file, err: error });
      });
  };
